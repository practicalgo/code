...
func TestSpawnExec(t *testing.T) {
	...
	defer cancel2()
	ctxCancInt, cancel, sCancel := ctxCancellableTimeout()
	defer cancel()
	defer sCancel()
	...
	tests := []testConfig{
		...
		testConfig{
			...
			ctx:             ctxCancInt,
			args:            []string{"10"},
			injectIntSignal: true,
			result: expectedResult{
				outputRegexp: regexp.MustCompile("go version go1.[0-9]+"),
				err:          fmt.Errorf("signal: interrupt"),
			},
		},
	}
	...
	for _, tc := range tests {
		...
		if tc.result.err != nil {
			if err == nil || strings.Index(err.Error(), tc.result.err.Error()) == -1 {
