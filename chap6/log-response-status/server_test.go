...
func TestMiddleware(t *testing.T) {
	...
	if resp.StatusCode != http.StatusInternalServerError {
		t.Errorf("Expected response status to be: %v, Got: %v", http.StatusInternalServerError, resp.StatusCode)
	}

	logs := b.String()
	expectedLogFragments := []string{
		"path=/panic method=GET duration=",
		"panic detected when handling request:",
	}
	for _, log := range expectedLogFragments {
		if !strings.Contains(log, log) {
