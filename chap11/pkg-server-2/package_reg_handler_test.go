...
func TestPackageRegHandler(t *testing.T) {
	...
	expectedPackageIdSubstring := "mypackage-0.1-mypackage-0.1.tar.gz"
	if !strings.Contains(resp.ID, expectedPackageIdSubstring) {
		t.Fatalf(
			"Expected version to contain %s, Got: %s\n",
			expectedPackageIdSubstring,
			resp.ID,
		)
	}
}
